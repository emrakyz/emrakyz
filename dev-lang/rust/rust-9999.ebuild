...
src_prepare() {
	...
	rm -f "${WORKDIR}/rust-patches-${RUST_PATCH_VER}/"*musl-dynamic-linking*.patch
...
src_install() {
	...
	# bug #689562, #689160
	rm -rfv "${ED}/usr/lib/${PN}/${SLOT}/etc/bash_completion.d/cargo" 2>/dev/null || true
	rm -rfv "${ED}/usr/lib/${PN}/${SLOT}"/etc{/bash_completion.d,} 2>/dev/null || true
